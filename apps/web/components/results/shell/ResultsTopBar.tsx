...
export function ResultsTopBar({ category, plSwing, accuracy }: ResultsTopBarProps) {
  const pillStyle = SITUATION_PILLS[category];
  const isPositive = plSwing >= 0;

  const handleFeedbackClick = () => {
    // FeedbackToggle lives inside VerdictSection in the overview section.
    // Scroll to overview, then try to click the feedback toggle.
    const el = document.querySelector('[data-section="overview"]');
    if (el) {
      el.scrollIntoView({ behavior: 'smooth', block: 'start' });
      // After scroll completes, find and click the feedback toggle
      setTimeout(() => {
        const toggle = document.querySelector('[data-feedback-toggle]') as HTMLButtonElement | null;
